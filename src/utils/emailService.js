    ...
    html: `
      ...
      <head>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px; }
          .header { border-bottom: 2px solid #f44336; padding-bottom: 10px; margin-bottom: 20px; }
          .reply-content { background-color: #f9f9f9; padding: 20px; border-radius: 5px; white-space: pre-wrap; }
          .footer { margin-top: 30px; font-size: 12px; color: #666; border-top: 1px solid #eee; padding-top: 10px; }
        </style>
      </head>
      ...
      <body>
        <div class="container">
          ...
          <div class="reply-content">
            ...
            <p style="font-size: 32px; font-weight: bold; color: #2196F3; letter-spacing: 8px;">${otp}</p>
...
const sendB2CVerificationEmail = async (b2cUser, verificationToken) => {
  const transporter = createTransporter()

  const verificationUrl = `${process.env.BACKEND_URL || "https://api.voyagian.com"}/api/b2c/verify-email/${verificationToken}`

  const mailOptions = {
    from: `"${process.env.SMTP_FROM_NAME || "Admin"}" <${process.env.SMTP_USER}>`,
    to: b2cUser.email,
    subject: "Verify Your Email - Voyagian",
    html: `
      <!DOCTYPE html>
      <html>
      <head>
        <style>
          ...
          .content { background-color: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px; }
          .button { display: inline-block; background-color: #667eea; color: white; padding: 14px 35px; text-decoration: none; border-radius: 5px; margin-top: 20px; font-weight: bold; }
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <p>Hi <strong>${b2cUser.name}</strong>,</p>
            <p>Thank you for registering with Voyagian! To complete your signup, please verify your email address by clicking the button below:</p>
            <p style="text-align: center;">
            ...
            <p>If the button above doesn't work, copy and paste this link into your browser:</p>
            <p style="word-break: break-all; color: #666; font-size: 12px;">${verificationUrl}</p>
            <p>Best regards,<br>The Voyagian Team</p>
          </div>
          <div class="footer">
            <p>This is an automated message. Please do not reply directly to this email.</p>
          </div>
  ...
  try {
    const info = await transporter.sendMail(mailOptions)
    console.log("B2C verification email sent:", info.messageId)
    return { success: true, messageId: info.messageId }
  } catch (error) {
    console.error("Error sending B2C verification email:", error)
    return { success: false, error: error.message }
  }
}

const sendB2CVerificationOTPEmail = async (email, otp, userName) => {
  const transporter = createTransporter()

  const mailOptions = {
    from: `"${process.env.SMTP_FROM_NAME || "Admin"}" <${process.env.SMTP_USER}>`,
    to: email,
    subject: "Your Email Verification Code - Voyagian",
    html: `
      <!DOCTYPE html>
      <html>
      <head>
        <style>
          ...
          .header { background-color: #667eea; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; }
          .content { background-color: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px; }
          .otp-box { background-color: #fff; border: 2px dashed #667eea; padding: 20px; text-align: center; margin: 20px 0; border-radius: 8px; }
  ...
  try {
    const info = await transporter.sendMail(mailOptions)
    console.log("B2C verification OTP email sent:", info.messageId)
    return { success: true, messageId: info.messageId }
  } catch (error) {
    console.error("Error sending B2C verification OTP email:", error)
    return { success: false, error: error.message }
  }
}

const sendB2CWelcomeEmail = async (b2cUser) => {
  const transporter = createTransporter()

  const mailOptions = {
    from: `"${process.env.SMTP_FROM_NAME || "Admin"}" <${process.env.SMTP_USER}>`,
    to: b2cUser.email,
    subject: "Welcome to Voyagian!",
    html: `
      <!DOCTYPE html>
      <html>
      <head>
        <style>
          ...
          .content { background-color: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px; }
          .button { display: inline-block; background-color: #667eea; color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; margin-top: 20px; }
          .footer { margin-top: 30px; font-size: 12px; color: #666; border-top: 1px solid #eee; padding-top: 10px; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            <h1>Welcome to Voyagian!</h1>
          </div>
          <div class="content">
            <p>Hi <strong>${b2cUser.name}</strong>,</p>
            <p>Your email has been verified successfully! You can now log in to your Voyagian account and start using our services.</p>
            <p><strong>Account Details:</strong></p>
            <ul>
              <li><strong>Name:</strong> ${b2cUser.name}</li>
              <li><strong>Email:</strong> ${b2cUser.email}</li>
  ...
  try {
    const info = await transporter.sendMail(mailOptions)
    console.log("B2C welcome email sent:", info.messageId)
    return { success: true, messageId: info.messageId }
  } catch (error) {
    console.error("Error sending B2C welcome email:", error)
    return { success: false, error: error.message }
  }
}
