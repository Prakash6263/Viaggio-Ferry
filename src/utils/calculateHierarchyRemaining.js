...
async function calculateHierarchyRemaining(params) {
  ...
  try {
    ...
    // STEP 2: Get allocations for each partner
    // We track the MINIMUM available at each level (bottleneck approach)
    let minAvailableAcrossLevels = companyRemaining
    ...
    for (const partnerIdLevel of partnerChain) {
      ...
      // Track the minimum available at this level
      // (the bottleneck is what limits the partner's access)
      minAvailableAcrossLevels = Math.min(minAvailableAcrossLevels, remaining)
    ...
    // Final available seats = minimum across all hierarchy levels (bottleneck)
    // This ensures we never oversell at any level
    const finalAvailableSeats = Math.max(0, minAvailableAcrossLevels)
    ...
    console.error("[v0] Hierarchy calculation error:", err.message)
    return {
      availableSeats: companyRemaining,
      availabilityBreakdown,
      finalAvailableSeats: companyRemaining,
