...
const { assignB2CUserToSellingPartner } = require("../utils/partnerHelper")
...
const registerB2CUser = async (req, res, next) => {
  try {
    ...
    if (!name || !email || !phone || !password || !companyName) {
      throw createHttpError(400, "Missing required fields: name, email, phone, password, companyName")
    }
    ...
    // This block handles auto-assigning the B2C user to a selling partner if a company is associated.
    // It's wrapped in a try-catch to prevent registration failure if partner assignment fails.
    if (companyId) {
      try {
        await assignB2CUserToSellingPartner(b2cUser._id, companyId)
      } catch (partnerError) {
        console.error("[v0] Warning: Could not auto-assign B2C user to Selling partner:", partnerError.message)
        // Don't fail registration if partner assignment fails
      }
    }
...
const loginB2CUser = async (req, res, next) => {
  try {
    ...
    if (!email || !password || !companyName) {
      throw createHttpError(400, "Email, password, and company name are required")
    }
    ...
    if (!company) {
      throw createHttpError(404, `Company '${companyName}' not found`)
    }
    ...
    const query = {
      email: email.toLowerCase(),
      company: companyId, // User must be registered with this company
    }

    const b2cUser = await B2CCustomer.findOne(query)
    if (!b2cUser) {
      throw createHttpError(401, "Invalid email or password, or you are not registered with this company")
    }
...
const getAllB2CUsersByCompany = async (req, res) => {
  try {
    ...
    const users = await B2CCustomer.find(filter)
      ...
      .populate("partner", "name") // Populated partner name along with company name for the B2C user list API
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(pageSize)
      .lean()
