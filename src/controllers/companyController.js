...
const { initializeCompanyLedgers } = require("../utils/ledgerHelper")
const { initializeDefaultB2CPartners } = require("../utils/partnerHelper")
...
const registerCompany = async (req, res, next) => {
  try {
    ...
    try {
      await initializeCompanyLedgers(company._id)
    } catch (ledgerError) {
      console.error("[v0] Warning: Could not initialize company ledgers:", ledgerError.message)
    }

    try {
      await initializeDefaultB2CPartners(company._id)
    } catch (partnerError) {
      console.error("[v0] Warning: Could not initialize default partners:", partnerError.message)
      // Don't fail registration if partner initialization fails - can be retried
    }
...
const deleteCompany = async (req, res, next) => {
  try {
    ...
    const company = await Company.findByIdAndUpdate(
      ...
      {
        isDeleted: true,
        isActive: false, // Also deactivate to ensure no access
      },
      { new: true },
    )
...
const confirmVerification = async (req, res, next) => {
  try {
    ...
    // In case the initial attempt during registration failed
    const existingLedgerCount = await CompanyLedger.countDocuments({ company: company._id })
    if (existingLedgerCount === 0) {
      try {
        await initializeCompanyLedgers(company._id)
      } catch (ledgerError) {
        console.error("[v0] Warning: Could not initialize company ledgers on verification:", ledgerError.message)
      }
    }

    try {
      const existingMarine = await Partner.countDocuments({
        company: company._id,
        layer: "Marine",
        isDeleted: false,
      })
      ...
      console.error("[v0] Warning: Could not initialize default partners on verification:", partnerError.message)
    }
...
const getCompanyPublicAboutByName = async (req, res, next) => {
  try {
    ...
    const company = await Company.findOne({
      ...
      status: "approved",
      isActive: true,
      isDeleted: false, // Added check for soft-deleted companies
    }).select(
...
const getCompanyBySlug = async (req, res, next) => {
  try {
    ...
    const company = await Company.findOne({
      ...
      status: "approved",
      isActive: true,
      isDeleted: false, // Ensure company is not soft-deleted
    }).select("companyName companySlug logoUrl whoWeAre whoWeAreImage vision mission purpose emailAddress website")

    // Always return 200 - never throw 404
    if (!company) {
      return res.status(200).json({
        success: true,
        exists: false,
