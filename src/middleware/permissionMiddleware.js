...
const checkPermission = (moduleCode, submoduleCode, action) => {
  return async (req, res, next) => {
    try {
      ...
      // CRITICAL: Check if user is super admin FIRST - before validating companyId
      // Super admins don't have companyId and don't need to validate it
      if (user?.role === "super_admin") {
        req.userPermission = {
          moduleCode,
          submoduleCode,
      ...
      // Validate authentication - only for non-admin roles
      if (!companyId) {
        throw createHttpError(401, "Company ID not found in token")
      }
